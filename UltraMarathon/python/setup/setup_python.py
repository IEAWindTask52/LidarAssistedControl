...
Usage:
    python setup/setup_python.py     (run from the UltraMarathon/python folder)

This creates a virtual environment in UltraMarathon/python/venv (kept local
to this project so it never touches your system/global Python) and installs
...
'../data/...'), so run it (or let your IDE run it) with
UltraMarathon/python/ as the working directory.

If you'd rather run RunUltraMarathon.py by clicking "Run" in an IDE, this
script also prints the venv's Python path at the end, plus where to
paste it in VS Code and PyCharm.
"""
...
THIS_DIR = os.path.dirname(os.path.abspath(__file__))          # .../UltraMarathon/python/setup
PYTHON_DIR = os.path.dirname(THIS_DIR)                          # .../UltraMarathon/python
...
def main():
    ...
    print()
    print('Setup complete. From UltraMarathon/python/, activate the environment with:')
    if os.name == 'nt':
