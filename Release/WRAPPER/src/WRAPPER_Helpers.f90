...
! DS: Few changes to be used for compiling the FFP DLLs: 1) replace Rosco_Types with WRAPPER_Types, 2) replace ErrorVariables with LidarErrorVariables, 3) add DbKi, IntKi, NewLine, PathSep
MODULE WRAPPER_Helpers

    USE, INTRINSIC :: ISO_C_Binding
    USE WRAPPER_Types 	! DS: ROSCO_Types renamed into WRAPPER_Types
    ! USE CONSTANTS ! DS: only add DbKi and IntKi from Constants.f90 for simplicity
    ! USE SysSubs	! DS: only add NewLine and PathSep from SysSubs (SysGnuWin.f90) for simplicity
	
	IMPLICIT NONE
	! DS: Variables from Constants.f90 and SysGnuWin.f90
