...
%% DS: Pitch Actuator
R.PA_CornerFreq = GetFASTPar(P.SD_dllP,'PA_CornerFreq');  
R.PA_Damping    = GetFASTPar(P.SD_dllP,'PA_Damping');       
F_PitchAct      = Af_LPF(R.PA_CornerFreq,R.PA_Damping,simu.dt); 
F.F_PitchAct.b  = F_PitchAct.num{1};
F.F_PitchAct.a  = F_PitchAct.den{1};

%% DS: Lidar-Assisted Control 
R.FlagLAC       = GetFASTPar(P.SD_dllP,'FlagLAC'); 

%% DS: add LPF for gain scheduling
F_P             = Af_LPF(R.F_LPFCornerFreq*0.25,0.7,simu.dt);
F.P.b           = F_P.num{1};
F.P.a           = F_P.den{1};